...
/**
 * Execute un cycle complet, ou un seul site si siteId est fourni (check
 * manuel de failover depuis le dashboard). Ne throw jamais (isolation par module).
 */
async function runCycle(trigger = 'planifié', siteId = null) {
  if (cycleInProgress) {
  ...
  log('info', 'runner', `Cycle démarré (${trigger}${siteId ? ' : ' + siteId : ''})`);

  try {
    let scrapers = registry.getScrapers(config);
    if (siteId) scrapers = scrapers.filter((s) => s.site.id === siteId);
